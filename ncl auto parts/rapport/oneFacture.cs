    ...
    public partial class oneFacture : Form
    {
        ...
        private async void oneFacture_Load(object sender, EventArgs e)
        {
            if (page == "auto")
            {
                ...
                MySqlDataAdapter dataAdapter = new MySqlDataAdapter("select *,(select sum(total) from facture_auto where no_recu='" + id + "') as real_total from facture_auto where no_recu='" + id + "'", connection);
