    ...
    public partial class oneFacture : Form
    {
        ...
        private async void oneFacture_Load(object sender, EventArgs e)
        {
            if (page == "auto")
            {
                ...
                MySqlDataAdapter dataAdapter = new MySqlDataAdapter("select *,(select total * 1 from facture_auto where no_recu='" + id + "' group by no_recu) as real_total from facture_auto where no_recu='" + id + "'", connection);
            ...
            else
            {
                FactureData vente = new FactureData();
                MySqlConnection connection = await dbConfig.connection();
                MySqlDataAdapter dataAdapter = new MySqlDataAdapter("select *,(select total * 1 from facture_garage where no_recu='" + id + "' group by no_recu) as real_total from facture_garage where no_recu='" + id + "';", connection);
                dataAdapter.Fill(vente, vente.Tables[0].TableName);
                ReportDataSource rds = new ReportDataSource("oneFacture", vente.Tables[0]);
                this.reportViewer1.LocalReport.DataSources.Clear();
                this.reportViewer1.LocalReport.DataSources.Add(rds);
