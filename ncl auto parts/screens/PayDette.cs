        ...
        private async void facture_Click(object sender, EventArgs e)
        {
            ...
            if (isAnumber) 
            {
                    ...
                    else
                    {
                        if (real_montant == 0)
                        {
                            int rep = await dbConfig.execute_command("update facture_auto set dette=" + real_montant + ",statut='paye' where no_recu='" + ID.Text + "'");
                            if (myDevise == "US")
                            {
                        ...
                        if (real_montant > 0)
                        {
                            int rep = await dbConfig.execute_command("update facture_auto set dette=" + real_montant + ",statut='avance' where no_recu='" + ID.Text + "'");
                            if (myDevise == "US")
                            {
                                VenteC.AddUsMoney(float.Parse(montant.Text));
                            }
                    ...
                    else
                    {
                        if (real_montant == 0)
                        {
                            int rep = await dbConfig.execute_command("update facture_garage set dette=" + real_montant + ",statut='paye' where no_recu='" + ID.Text + "'");
                            if (myDevise == "US")
                            {
                        ...
                        if (real_montant > 0)
                        {
                            int rep = await dbConfig.execute_command("update facture_garage set dette=" + real_montant + ",statut='avance' where no_recu='" + ID.Text + "'");
                            if (myDevise == "US")
                            {
                                VenteC.AddUsMoneyGarage(float.Parse(montant.Text));
                            }
